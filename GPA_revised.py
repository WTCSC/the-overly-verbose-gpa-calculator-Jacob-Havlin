...
l=[]
t=int(I("How many classes would you like to calculate? "))
for i in range(t):
    while True:
        try:
            g=float(I(f"Please enter your grade (0.0-5.0) for class {i+1} "))
...
while True:
    try:
        s=int(I("Which semester would you like to focus on? "))
...
while True:
    try:
        goal=float(I("What is your goal GPA? "))
        if 0<=goal<=5:
            if a>=goal:
                p("Congratulations you have already met this goal")
                if I("Would you like to set a different goal? (yes/no) ").lower()=="no":p("Keep up the good work");exit()
            else:break
